...
export default function SpinningWheel() {
  ...
  const spinWheel = () => {
    ...
    const randomRotations = Math.floor(Math.random() *
  (maxRotations - minRotations + 1)) + minRotations
...
const viewSelectedTopic = () => {
  if (selectedTopic) {
    router.push(`/topics/${selectedTopic.id}`)
  }
}
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-[500px]">
        ...
        <p claseName="mt-4 text-slate-600 dark:text-slate-300">Loading Topics...</p>
        </div>
    )
  }
return (
  <div className="flex flex-col items-center">
    <div className="relative w-full max-w-lg aspect-square">
      {/* Pointer/Arrow */}
      <div className="absolute z-10 w-8 h-16 - translate-x-1/2 -translate-y-1/2 bg-red-500
      left-1/2 top-0 clip-path-triangle"></div>
...
{topics.map((topic, index) => {
  ...
  return (
    ...
    h-1/2 origin-bottom-left text-white font-medium flex items-center
...
{selectedTopic && (
  <div className="p-4 mt-4 text-center">
    ...
    <p className="mb-4 text-slate-600 dark:text-slate-300">{selectedTopic.shortDescription}</p>
